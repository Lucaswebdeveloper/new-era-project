			...
			request.onload = function () {
				if (this.status >= 200 && this.status < 400) {
					...
					if ($('#owl-carousel-instagram').length) {
						var owl = $('#owl-carousel-instagram');
						owl.owlCarousel({
							stagePadding: 100,
							loop: true,
							autoplay: 3000,
		...
		request.onload = function () {
			if (this.status >= 200 && this.status < 400) {
				...
				for (const i in itens) {
					...
					html += `<div class="col-xs-4 d-flex espaco-banner">
...
/*carrossel videos | MOBILE*/

if ($('#owl-carousel-videos').length) {
...
/*carrossel 3 CARDS | MOBILE*/

if ($('#owl-carousel-card').length) {
...
/*carrossel FAÃ‡A PARTE ASSOCIADOS | MOBILE*/

if ($('#owl-carousel-faca-parte').length) {
	var owl = $('#owl-carousel-faca-parte');
	owl.owlCarousel({
		stagePadding: 50,
		loop: true,
		margin: 10,
