...
$(document).ready(function() {
			...
			try {
				...
				request.onload = function() {
					if (this.status >= 200 && this.status < 400) {
						...
						html+=`<div style="text-align: center;"><a href="/noticias">VER TODAS</a></div>`;
				...
				if (this.status >= 200 && this.status < 400) {
					...
					for (i in materias) {
						if(i <= 3){
							...
							if( materias[i].media_url.includes('video')){
								html += '	<video style="object-fit: cover; height: 356px; width: 356px;" src="' + materias[i].media_url + '" alt="">';
							} else {
	...
	if((convenios = jQuery('#informacoes-convenios-lista')).length > 0){
		var urlConvenios = "https://www.anajustra.org.br/api/convenios/top10.ashx",
		request = new XMLHttpRequest();
	...
	if((convenios = jQuery('#informacoes-convenios-lista-mobile')).length > 0){
		var urlConvenios = "https://www.anajustra.org.br/api/convenios/top10.ashx",
		request = new XMLHttpRequest();
		request.open("GET", urlConvenios, true);		
		request.onload = function() {
			if (this.status >= 200 && this.status < 400) {
				...
				for (const i in itens) {
					var {DESCRICAO, IMAGEM, LINK, POSICAO} = itens[i],
					_LINK = (LINK == '#') ? 'https://anajustrabeneficios.com.br/' : LINK;
					html += `
					<li id="carousel__slide${toString(POSICAO)}" tabindex="0" data-href="${toString(POSICAO)}" class="carousel__slide">                                        
                       <div class="carousel__snapper">
                            <div style="padding: 30px 70px;">
                                ...
                                <a href="#carousel__slide${POSICAO === 1 ? 10 : POSICAO - 1}" class="carousel__prev">Go to last slide</a>
                                <a href="#carousel__slide${POSICAO === 10 ? 1 : POSICAO + 1} class="carousel__next">Go to next slide</a>   
                            </div>   
                        </div>              
                    </li>`;
					if(count == 3) break;
